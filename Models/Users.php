...
class Users extends Model
{
	...
	public function addUser($name, $email, $password, $type_user, $id_group)
	{
		$sql = $this->db->prepare("INSERT INTO users SET name = :name, email = :email, password = :password, type = :type_user, id_group = :id_group, situation = '1'");
	...
	public function doLoginWithAutoHash($email, $password)
{
    // Pega usuário pelo e-mail
    $sql = $this->db->prepare("SELECT id, password FROM users WHERE email = :email");
    $sql->bindValue(":email", $email);
    $sql->execute();
    ...
    if ($sql->rowCount() > 0) {
        ...
        // Debug
        var_dump("Senha do banco:", $row["password"]);

        // Se a senha no banco não estiver hashada, cria hash automaticamente
        if (!password_get_info($row["password"])['algo']) {
            $hash = password_hash($password, PASSWORD_DEFAULT);
            $update = $this->db->prepare("UPDATE users SET password = :pass WHERE id = :id");
            $update->bindValue(':pass', $hash);
            $update->bindValue(':id', $row['id']);
            $update->execute();

            $_SESSION[CONF_SESSION_NAME] = $row["id"];
            var_dump("Senha convertida para hash automaticamente!");
            return true;
        }
